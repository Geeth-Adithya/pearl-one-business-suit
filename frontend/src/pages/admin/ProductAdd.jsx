...
import { Package, Upload, ArrowLeft, Save, Loader2, Plus, Image as ImageIcon } from 'lucide-react';
...
export default function ProductAdd() {
  ...
  const [form, setForm] = useState({
    name: '', type: 'Single', attribute: '', item_code: '', search_keywords: '',
    purchasing_price: '', margin_percent: '', selling_price: '',
    stock_quantity: '', low_stock_threshold: '5', unit: 'pcs',
    description: '', supplier_name: '', image_url: '', category_ids: []
  });
    ...
    axios.get(`${API_URL}/products.php?action=form_data`)
      .then(res => {
        if (res.data.success) {
          ...
          setForm(f => ({ ...f, item_code: res.data.next_code }));
        }
      })
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await axios.post(`${API_URL}/products.php`, { ...form, action: 'create' });
      if (res.data.success) {
        navigate('/inventory');
      } else {
        alert(res.data.message);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-text-light dark:text-white flex items-center gap-3">
              <Plus className="text-brand-500" size={26} />
              Add New Product
            </h1>
            <p className="text-text-muted dark:text-text-mutedDark text-sm mt-1">Create a new item in your inventory.</p>
          </div>
        </div>
      </div>
          ...
          <div className="glass-card p-6 shadow-xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input name="name" value={form.name} onChange={handleChange} required className={inputCls} placeholder="e.g. Wireless Mouse" />
              </div>
              
              <div>
                <label className={labelCls}>Item Code</label>
                <input name="item_code" value={form.item_code} onChange={handleChange} className={inputCls} placeholder="Auto-generated if empty" />
              </div>
              ...
              {form.type === 'Variant' && (
                <div className="md:col-span-2">
                  ...
                  <input name="attribute" value={form.attribute} onChange={handleChange} className={inputCls} placeholder="e.g. Size M, Red Color" />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="glass-card p-6 shadow-xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input type="number" step="0.01" min="0" name="purchasing_price" value={form.purchasing_price} onChange={handleChange} className={inputCls} placeholder="0.00" />
              </div>
              <div>
                <label className={labelCls}>Margin (%)</label>
                <input type="number" step="0.01" name="margin_percent" value={form.margin_percent} onChange={handleChange} className={inputCls} placeholder="0%" />
              </div>
              <div>
                <label className={labelCls}>Selling Price (Rs.) <span className="text-red-500">*</span></label>
                <input type="number" step="0.01" min="0" name="selling_price" value={form.selling_price} onChange={handleChange} required className={inputCls} placeholder="0.00" />
              </div>

              <div>
                <label className={labelCls}>Initial Stock</label>
                <input type="number" min="0" name="stock_quantity" value={form.stock_quantity} onChange={handleChange} className={inputCls} placeholder="0" />
              </div>
              <div>
                <label className={labelCls}>Low Stock Alert At</label>
                <input type="number" min="0" name="low_stock_threshold" value={form.low_stock_threshold} onChange={handleChange} className={inputCls} placeholder="5" />
              </div>
              <div>
                <label className={labelCls}>Unit</label>
                <input name="unit" value={form.unit} onChange={handleChange} className={inputCls} placeholder="pcs, kg, m..." />
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-6 shadow-xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <input name="search_keywords" value={form.search_keywords} onChange={handleChange} className={inputCls} placeholder="e.g. logitech, bluetooth, wireless" />
              </div>

              <div className="md:col-span-2">
                <label className={labelCls}>Description</label>
                <textarea name="description" value={form.description} onChange={handleChange} rows="3" className={inputCls} placeholder="Short product description..."></textarea>
              </div>
            </div>
          </div>
